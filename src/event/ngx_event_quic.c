...
//got initial packet,init conn to quic and c
ngx_int_t
ngx_quic_create_connection(ngx_quic_t *quic, ngx_connection_t *c)
{
    ...
    if (!quiche_version_is_supported(pkt_version)) {
        ...
        //use the api in quiche.h.just included in ngx_quiche.h
...
//do maybe after this the handshake is done
ngx_int_t
...
//DTPnginx
    ...
    c->log->action = "reading DTP packets";
 

    ngx_log_debug0(NGX_LOG_DEBUG_EVENT, c->log, 0, out);

    if (rev->timedout) {
        ngx_log_debug0(NGX_LOG_DEBUG_EVENT, c->log, 0,
                       "DTP connection timed out");
...
//DTPnginx
            #if (NGX_DTP)
              ngx_log_debug1(NGX_LOG_DEBUG_EVENT, c->log, 0,
                              "failed to process DTP packet: %d", done);
            #elif (NGX_QUIC)
                 ngx_log_debug1(NGX_LOG_DEBUG_EVENT, c->log, 0,
                              "failed to process quic packet: %d", done);
            #endif
...
//DTPnginx
    #if (NGX_DTP)
         ngx_log_debug0(NGX_LOG_DEBUG_EVENT, c->log, 0, "DTP done reading");
    #elif (NGX_QUIC)
