...
ngx_http_v3_send_response(ngx_http_request_t *r)
{
    ...
    c = h3c->connection;
    //no work related to quic protocol itself
...
ngx_http_v3_stream_do_send(ngx_connection_t *fc, ngx_buf_t *b, ngx_int_t fin)
{
    ...
    if (!stream->headers_sent) {
        ...
        n  = quiche_h3_send_body_block(h3c->h3, c->quic->conn, r->qstream->id,
                            buf, buf_len, fin,&block);
        conn_io->send_round++;
        if (n != block.size) {
        ngx_log_debug3(NGX_LOG_DEBUG_EVENT, fc->log, 0,"failed to send block %d completely: sent %d",
                conn_io->send_round,  n);
    }
